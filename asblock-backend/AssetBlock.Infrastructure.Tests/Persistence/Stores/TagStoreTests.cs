...
public sealed class TagStoreTests
{
    ...
    public async Task Add_GetById_GetByName_GetTagsByNames_SearchTags_GetPaged()
    {
        ...
        List<Tag> byNames = await sut.GetTagsByNames(["unity", "missing"]);
        byNames.Should().HaveCount(1);

        PagedResult<Tag> paged = await sut.SearchTags(new GetTagsRequest { Page = 1, PageSize = 10 });
        paged.TotalCount.Should().BeGreaterThanOrEqualTo(1);

        PagedResult<Tag> pagedById = await sut.SearchTags(new GetTagsRequest { SortBy = "id", SortDirection = SortDirection.DESC });
