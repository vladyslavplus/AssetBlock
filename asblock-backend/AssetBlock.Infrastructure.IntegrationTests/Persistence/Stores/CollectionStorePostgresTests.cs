...
public sealed class CollectionStorePostgresTests(PostgresFixture fixture)
{
    ...
    public async Task AddItem_WhenAssetAlreadyInCollection_ShouldThrowDuplicateCollectionAssetException()
    {
        ...
        Collection collection = await store.Create(author.Id, "Picks", null);
        await store.AddItem(collection.Id, asset.Id);
        db.ChangeTracker.Clear();

        Func<Task> act = () => store.AddItem(collection.Id, asset.Id);
    ...
    public async Task CollectionItem_WhenPositionDuplicated_ShouldViolateUniqueConstraint()
    {
        ...
        Collection collection = TestData.CreateCollection(author.Id, status: CollectionStatus.DRAFT);
    ...
    public async Task GetPublicDetail_WhenSomeItemsSoftDeleted_ShouldHideThemAndRenumber()
    {
        await using ApplicationDbContext db = await fixture.CreateCleanDbContext();
        (User author, Category category) = await TestData.SeedAuthorAndCategory(db);
        Asset kept = TestData.CreateAsset(author.Id, category.Id, title: "Kept");
        Asset gone = TestData.CreateAsset(author.Id, category.Id, title: "Gone");
        db.Assets.AddRange(kept, gone);
        await db.SaveChangesAsync();
        db.AssetVersions.AddRange(
            TestData.CreateAssetVersion(kept.Id),
            TestData.CreateAssetVersion(gone.Id));
        Collection collection = TestData.CreateCollection(author.Id, status: CollectionStatus.PUBLISHED);
        db.Collections.Add(collection);
        ...
        CollectionDetailDto? detail = await new CollectionStore(db).GetPublicDetail(collection.Id);
    ...
    public async Task HardDeleteCollection_ShouldCascadeCollectionItems()
    {
        ...
        await db.SaveChangesAsync();
        db.AssetVersions.Add(TestData.CreateAssetVersion(asset.Id));
        Collection collection = TestData.CreateCollection(author.Id);
        db.Collections.Add(collection);
        db.CollectionItems.Add(TestData.CreateCollectionItem(collection.Id, asset.Id, position: 1));
        await db.SaveChangesAsync();
    ...
    public async Task RemoveItem_WhenRemovingFirstOrMiddle_ShouldKeepContiguousPositivePositions()
    {
        await using ApplicationDbContext db = await fixture.CreateCleanDbContext();
        (User author, Category category) = await TestData.SeedAuthorAndCategory(db);
        Asset assetA = TestData.CreateAsset(author.Id, category.Id, title: "Alpha");
        Asset assetB = TestData.CreateAsset(author.Id, category.Id, title: "Beta");
        Asset assetC = TestData.CreateAsset(author.Id, category.Id, title: "Gamma");
        ...
        List<CollectionItem> afterFirst = await db.CollectionItems.AsNoTracking()
        ...
        List<CollectionItem> afterMiddle = await db.CollectionItems.AsNoTracking()
            .Where(i => i.CollectionId == collection.Id)
            .OrderBy(i => i.Position)
            .ToListAsync();
    ...
    public async Task ReorderItems_WhenNonEmpty_ShouldSucceedWithPositivePositions()
    {
        await using ApplicationDbContext db = await fixture.CreateCleanDbContext();
        (User author, Category category) = await TestData.SeedAuthorAndCategory(db);
        Asset assetA = TestData.CreateAsset(author.Id, category.Id, title: "First");
        Asset assetB = TestData.CreateAsset(author.Id, category.Id, title: "Second");
        Asset assetC = TestData.CreateAsset(author.Id, category.Id, title: "Third");
        db.Assets.AddRange(assetA, assetB, assetC);
        await db.SaveChangesAsync();
        db.AssetVersions.AddRange(
            TestData.CreateAssetVersion(assetA.Id),
        ...
        List<CollectionItem> items = await db.CollectionItems.AsNoTracking()
            .Where(i => i.CollectionId == collection.Id)
            .OrderBy(i => i.Position)
            .ToListAsync();
    ...
    {
        await using ApplicationDbContext setup = await fixture.CreateCleanDbContext();
        ...
        Task removeTask = RemoveFirst();
        await firstHasLock.Task;
        Task reorderTask = ReorderAfterRemove();
        ...
        List<CollectionItem> items = await verify.CollectionItems
            .AsNoTracking()
            .Where(i => i.CollectionId == collection.Id)
            .OrderBy(i => i.Position)
            .ToListAsync();
