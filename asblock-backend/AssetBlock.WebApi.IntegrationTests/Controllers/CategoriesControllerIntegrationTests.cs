...
public sealed class CategoriesControllerIntegrationTests(IntegrationTestFixture fixture)
{
    ...
    public async Task Get_WhenDefaults_ShouldReturnOkWithSeededItems()
    {
        ...
        HttpResponseMessage response = await client.GetAsync(new Uri("/api/categories?page=1&pageSize=20", UriKind.Relative));
    ...
    {
        HttpClient client = fixture.Factory.CreateClient();
        HttpResponseMessage response = await client.GetAsync(new Uri("/api/categories?search=Algorithms&page=1&pageSize=20", UriKind.Relative));

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        PagedCategoriesResponse? page = await response.Content.ReadFromJsonAsync<PagedCategoriesResponse>();
    ...
    {
        HttpClient client = fixture.Factory.CreateClient();
        HttpResponseMessage response = await client.GetAsync(new Uri("/api/categories?page=1&pageSize=10&sortBy=InvalidSort", UriKind.Relative));
    ...
    {
        HttpClient client = fixture.Factory.CreateClient();
        HttpResponseMessage response = await client.GetAsync(new Uri("/api/categories?page=1&pageSize=101", UriKind.Relative));
    ...
    public async Task GetById_WhenMissing_ShouldReturnNotFoundWithErrorCode()
    {
        ...
        HttpResponseMessage response = await client.GetAsync(new Uri($"/api/categories/{missingId}", UriKind.Relative));
    ...
    {
        HttpClient client = fixture.Factory.CreateClient();
        HttpResponseMessage listResponse = await client.GetAsync(new Uri("/api/categories?page=1&pageSize=1&sortBy=name&sortDirection=ASC", UriKind.Relative));
        listResponse.EnsureSuccessStatusCode();
        PagedCategoriesResponse? page = await listResponse.Content.ReadFromJsonAsync<PagedCategoriesResponse>();
        page.Should().NotBeNull();
        page.Items.Should().NotBeEmpty();

        Guid id = page.Items[0].Id;
        HttpResponseMessage response = await client.GetAsync(new Uri($"/api/categories/{id}", UriKind.Relative));

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        CategoryDetailResponse? detail = await response.Content.ReadFromJsonAsync<CategoryDetailResponse>();
