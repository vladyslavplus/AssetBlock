...
export function HeroInteractiveBackground({
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      const particleCount =
        window.innerWidth < MOBILE_BREAKPOINT_PX ? MOBILE_PARTICLE_COUNT : DESKTOP_PARTICLE_COUNT
    ...
    const renderFrame = (timestamp: number) => {
      ...
      for (const particle of particlesRef.current) {
        ...
        const driftX =
          Math.cos(driftTheta) * 0.06 + Math.sin(driftTheta * 0.47 + particle.driftPhase) * 0.032
        const driftY =
        ...
        if (pointer.inside) {
          ...
          if (distance < POINTER_RADIUS_PX && distance > 0.001) {
            ...
            const impulse =
              influence * influence * (POINTER_FORCE + pointerVelocity * POINTER_VELOCITY_FORCE)
      ...
      if (burstParticlesRef.current.length > 0) {
        ...
        for (const burstParticle of burstParticlesRef.current) {
          ...
          context.arc(
            burstParticle.x,
            burstParticle.y,
            burstParticle.size * burstParticle.life,
            0,
            Math.PI * 2,
          )
          context.fill()
