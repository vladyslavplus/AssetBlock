...
import { getNotificationAssetId, getNotificationBody, getNotificationTitle } from "@/lib/notifications/notification-ui";
import {
...
export function NotificationBell() {
  ...
  const readAllMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.setQueryData<InfiniteData<PagedNotificationsDto, number>>(notificationsKeys.inbox(), (old) =>
        markAllInboxItemsRead(old, now),
      );
      toast.success(
        data.updatedCount > 0
    ...
    onError: (err: unknown) => {
      const msg = err instanceof Error ? err.message : getApiErrorMessage(err, "Could not mark all as read.");
      toast.error(msg);
    },
  });
  ...
  const toggleRead = async (n: NotificationListItem) => {
    ...
    try {
      if (wasUnread) {
        ...
        queryClient.setQueryData<InfiniteData<PagedNotificationsDto, number>>(notificationsKeys.inbox(), (old) =>
          updateInboxItemReadAt(old, n.id, null),
        );
      }
      void queryClient.invalidateQueries({ queryKey: notificationsKeys.unread() });
    } catch (err) {
      toast.error(
        err instanceof Error ? err.message : wasUnread ? "Could not mark as read." : "Could not mark as unread.",
      );
    }
  };
        ...
        >
          <div className="shrink-0 px-3 py-2 border-b border-border flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-0.5 shrink-0">
              ...
              >
                {listBusy ? <Loader2 className="size-4 animate-spin" aria-hidden /> : <RotateCw className="size-4" aria-hidden />}
              </Button>
            </div>
          </div>
