...
export interface AssetsBrowsePageProps {
  initialFilters?: CatalogFilters
  initialAssetsResult?: FetchAssetsPageResult | null
  initialFacets?: CatalogFacets | null
}

function areFiltersEqual(a?: CatalogFilters, b?: CatalogFilters): boolean {
  if (!a || !b) return false
  return (
    a.page === b.page &&
...
function AssetsCatalogContent({
  ...
  const isInitialFilterMatch = areFiltersEqual(filters, initialFilters)

  const listQuery = useQuery({
    queryKey: catalogKeys.list(filters),
    queryFn: () => fetchCatalogPage(filters),
    initialData: isInitialFilterMatch && initialAssetsResult ? initialAssetsResult : undefined,
    staleTime: 60 * 1000,
  ...
  const handleResetFilters = () => {
    const qs = serializeCatalogUrlParams(DEFAULT_CATALOG_FILTERS).toString()
    const target = (qs ? `${pathname}?${qs}` : pathname) as Route
    router.push(target)
  }
...
export function AssetsBrowsePage(props: AssetsBrowsePageProps) {
  return (
    <Suspense>
      <AssetsCatalogContent {...props} />
    </Suspense>
  )
}
