...
function buildIdleGlitchCandidates(): Array<
  { scope: "line1"; index: number } | { scope: "prefix"; index: number } | { scope: "accent"; index: number }
> {
  const out: Array<
    { scope: "line1"; index: number } | { scope: "prefix"; index: number } | { scope: "accent"; index: number }
  > = [];
...
export function HeroTypewriterTitle({ prefersReducedMotion, className, style }: HeroTypewriterTitleProps) {
  ...
  useEffect(() => {
    ...
    const scheduleNextIdle = () => {
      ...
      const outerId = window.setTimeout(() => {
        ...
        const pick = IDLE_GLITCH_CANDIDATES[Math.floor(Math.random() * IDLE_GLITCH_CANDIDATES.length)];
        if (!pick) {
          scheduleNextIdle();
          return;
        }
      ...
      <span className="relative block">
        ...
        <span className="absolute inset-0 block" aria-hidden="true">
          ...
          <span className="block transition-opacity duration-300 ease-out">
            <span className={line2Prefix.length > 0 ? "opacity-100" : "opacity-0"}>{prefixShown || LINE2_PREFIX}</span>
