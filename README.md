...
**High-level capabilities:** marketing home with featured catalog strip, asset catalog with filters and pagination, asset detail (current version, license terms, content hash, version history) and checkout flow, authenticated library (purchased vs latest entitled version, update badge, version-specific download) and account settings (including email verification UX gates for marketplace actions), seller hub (listings, upload with license selection, edit metadata, publish new versions), admin UI for categories/tags/review moderation and audit log browse, lightweight docs page, login and registration.
